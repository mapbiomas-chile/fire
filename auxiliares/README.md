...
1. Por cada cicatriz original: `thr_i = max(p10(dNBR | cicatriz_i), 0.10)`
2. Candidatos de esa cicatriz = dNBR válido ≥ `thr_i`
3. Crecimiento conectado (8) solo desde esa cicatriz
4. Unión de todas las expansiones

Así cada incendio usa **su propio umbral** (no un p10 global del tile, que en años problemáticos bajaba demasiado el listón). Floor `MIN_DNBR=0.10`. Banda 13. Regiones `r1 r2 r4 r6`, 2019–2025.

```text
… → thr_i por componente (p10 de la cicatriz ya clasificada)
  → traversable_i = scar_i OR (dNBR>=thr_i)
  → label 8-conn; keep component touching scar_i
...
# Borrar/renombrar salida de la corrida anterior antes de relanzar
# Opción A — borrar todo:
rm -rf ~/classification_20260713_dnbr_expanded
# Opción B — conservar backup:
# mv ~/classification_20260713_dnbr_expanded ~/classification_20260713_dnbr_expanded_global_p10_bak
...
- `expand_stats.csv` — umbrales min/med/max por tile, conteos, hectáreas

| Parámetro | Default |
|-----------|---------|
| `--threshold-mode` | `per_component` (alternativa: `global`) |
| `--dnbr-band` | 13 |
| `--dnbr-percentile` | 10 |
| `--min-dnbr` | 0.10 |
| conectividad | 8 |
