#!/bin/bash
#---------------Script SBATCH - NLHPC ----------------
# Fill burn gaps: MODIS candidates ∩ dNBR similar to classified scars ∩ ~strict LULC
#
#   cd ~/fire && git pull
#   mkdir -p ~/logs
#   sbatch auxiliares/run_fill_modis_similar_slurm.sh
#
# Una región/año de prueba:
#   EXPAND_REGIONS=r1 EXPAND_FROM_YEAR=2020 EXPAND_TO_YEAR=2020 \
#     sbatch auxiliares/run_fill_modis_similar_slurm.sh
#
#SBATCH -J fire_modis_similar
#SBATCH -p main
#SBATCH -n 1
#SBATCH -c 8
#SBATCH --mem=64GB
#SBATCH --mail-type=ALL
#SBATCH -t 01:00:00
#SBATCH -o /home/%u/logs/%x_%j.out
#SBATCH -e /home/%u/logs/%x_%j.err

set -euo pipefail

FIRE_REPO="${REPO_ROOT:-${HOME}/fire}"
AUX_DIR="${FIRE_REPO}/auxiliares"
PATHS_FILE="${AUXILIARES_PATHS_FILE:-${AUX_DIR}/cluster_paths.env}"
SCRIPT="${AUX_DIR}/fill_burn_from_modis_similarity.py"

CLASS_DIR="${EXPAND_CLASS_DIR:-${HOME}/classification_20260713}"
MOSAIC_DIR="${EXPAND_MOSAIC_DIR:-${HOME}/mosaics_cog}"
MODIS_DIR="${MODIS_DIR:-${HOME}/MODIS}"
OUTPUT_DIR="${MODIS_SIMILAR_OUTPUT_DIR:-${HOME}/classification_20260713_modis_similar}"
MASCARAS_ROOT="${MASCARAS_ROOT:-${HOME}/classification_20260619/filtering_work/mascaras}"
REGIONS="${EXPAND_REGIONS:-r1 r2 r4 r6}"
FROM_YEAR="${EXPAND_FROM_YEAR:-2019}"
TO_YEAR="${EXPAND_TO_YEAR:-2025}"
DNBR_BAND="${EXPAND_DNBR_BAND:-13}"
MIN_DNBR="${EXPAND_MIN_DNBR:-0.10}"
MAD_K="${MODIS_MAD_K:-1.5}"
SKIP_EXISTING="${EXPAND_SKIP_EXISTING:-0}"

export OMP_NUM_THREADS="${OMP_NUM_THREADS:-8}"

echo "============================================="
echo "MODIS × dNBR similarity fill — NLHPC"
echo "============================================="
echo "Repo:      ${FIRE_REPO}"
echo "Class:     ${CLASS_DIR}"
echo "Mosaics:   ${MOSAIC_DIR}"
echo "MODIS:     ${MODIS_DIR}"
echo "Masks:     ${MASCARAS_ROOT}"
echo "Output:    ${OUTPUT_DIR}"
echo "Regions:   ${REGIONS} | ${FROM_YEAR}-${TO_YEAR}"
echo "mad_k=${MAD_K} min_dnbr=${MIN_DNBR}"
echo "Job id:    ${SLURM_JOB_ID:-local}"
echo "============================================="

mkdir -p "${HOME}/logs" "${OUTPUT_DIR}"

if [[ -f "${PATHS_FILE}" ]]; then
  # shellcheck source=/dev/null
  source "${PATHS_FILE}"
fi

PYTHON="${PYTHON:-${HOME}/.conda/envs/mb_fuego/bin/python}"

for d in "${CLASS_DIR}" "${MOSAIC_DIR}" "${MODIS_DIR}" "${MASCARAS_ROOT}"; do
  if [[ ! -d "${d}" ]]; then
    echo "ERROR: missing dir: ${d}" >&2
    exit 1
  fi
done

"${PYTHON}" -c "import numpy, pandas, rasterio; print('deps OK')"

cmd=(
  "${PYTHON}" "${SCRIPT}"
  --class-dir "${CLASS_DIR}"
  --mosaic-dir "${MOSAIC_DIR}"
  --modis-dir "${MODIS_DIR}"
  --output-dir "${OUTPUT_DIR}"
  --mascaras-root "${MASCARAS_ROOT}"
  --regions ${REGIONS}
  --from-year "${FROM_YEAR}"
  --to-year "${TO_YEAR}"
  --dnbr-band "${DNBR_BAND}"
  --min-dnbr "${MIN_DNBR}"
  --mad-k "${MAD_K}"
  --stats-csv "${OUTPUT_DIR}/modis_similar_stats.csv"
)

if [[ "${SKIP_EXISTING}" == "1" ]]; then
  cmd+=(--skip-existing)
fi

cd "${FIRE_REPO}"
echo "Running: ${cmd[*]}"
"${cmd[@]}"
echo "Done. Stats: ${OUTPUT_DIR}/modis_similar_stats.csv"
exit $?
