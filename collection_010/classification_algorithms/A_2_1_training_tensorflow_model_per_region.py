...
if 'TrainingInterface' not in globals():
    raise RuntimeError("[ERROR] TrainingInterface is not defined. Run modified A_2_0 first.")
        ...
        try:
            ...
            if cleaned.shape[0] == 0:
                ...
                self.log_message(
                    ...
                    f"{cleaned.shape[0]}/{vector.shape[0]} valid pixels"
                )
...
def infer_dataset_schema(sample_path, label_name="landcover"):
    ...
    input_band_indices = [
        i for i in range(band_count)
        if i != label_band_index
    ]

    input_band_names = [
        band_names[i]
        for i in input_band_indices
    ]
...
def sample_download_and_preparation(images_train_test):
    """
    Descarga, procesa y concatena únicamente las muestras seleccionadas
    manualmente en A_2_0.
    """
    ...
    log_message(
        f"[INFO] Starting image download and preparation for "
        f"{len(images_train_test)} selected images..."
    )
    ...
    with tqdm(
        total=len(images_train_test),
        desc="[INFO] Downloading and processing selected images"
    ) as pbar:
        ...
        for image in images_train_test:
            ...
            if os.path.exists(local_file):
                log_message(
                    f"[INFO] The file {image} already exists. "
                    f"Skipping download, but processing it."
                )
            else:
    ...
    trainer = ModelTrainer(
        ...
        get_active_checkbox_func=interface.get_active_checkbox
    )

    trainer.split_and_train(
        valid_data_train_test=valid_data_train_test,
